 ...
 * @param {Boolean} [options.mfDataAsProfile=false] - Prefer to have the parsed
       microformat data passed to the verify callback as `profile`. Default is
       to map the parsed data to PortableContacts schema
...
function IndieAuthStrategy(options, verify) {
  ...
  this._defaultAuthEndpoint = (
    options.defaultAuthEndpoint || 'https://indieauth.com/auth'
  );

  this._mfDataAsProfile = options.mfDataAsProfile || false;

  this._responseType = options.responseType || 'id';
  if (this._responseType !== 'id' && this._responseType !== 'code') {
...
Strategy.prototype.authenticate = function(req, options) {
  ...
  strategy
    ._parseUserPage(requestParams.me, function parseUserPageCB(err, mfData) {
      if (err) { return strategy.fail({ message: err.message, }); }
      
      // Fallback to configured default if endpoint not found
      if (!mfData.rels || !mfData.rels.authorization_endpoint) {
	authEndpoint = strategy._defaultAuthEndpoint;
      } else {
	authEndpoint = mfData.rels.authorization_endpoint[0];
      }
      
      // TODO: verify authEndpoint as valid URL
	...
	strategy._verifyAuthCode(authEndpoint, {
	  ...
	  if (strategy._passReqToCallback) {
	    return strategy._verify(
	      req, domain, scope, profile, strategy._verified.bind(strategy)
	    );
	  } else {
	    return strategy._verify(
	      domain, scope, profile, strategy._verified.bind(strategy)
	    );
	  }
	});
      }
...
function convertToProfile(mfData) {
  ...
  if (!mfData || !mfData.items.length) { return { provider: 'indieauth', }; }
  ...
  if (!properties) { return { provider: 'indieauth', }; }
 ...
 */
IndieAuthStrategy.prototype._verified = function(err, user, info) {
 ...
 */
IndieAuthStrategy.prototype._verifyAuthCode = function(url, params, callback) {
 ...
 */
IndieAuthStrategy.prototype._parseUserPage = function(url, callback) {
  ...
  request.get(requestUrl, function(err, response, body) {
    ...
    if (!body) {
      return callback(new TypeError('request to user page must return a body'), null);
    }
